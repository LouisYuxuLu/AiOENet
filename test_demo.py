	...
	if IsGPU == 1:
		for f in range(len(testfiles)):
			...
			with torch.no_grad():
				...
				h,w,c = img_c.shape
				img_cc = cv2.resize(img_c,(512,512)) / 255.0
				...
				s = time.time()
				E_out = model(input_var,Type=4)
				e = time.time()   
				print(input_var.shape)       
				print('Time:%.4f'%(e-s))    
				E_out = cv2.resize(chw_to_hwc(E_out.squeeze().cpu().detach().numpy()),(1600,1000))			               
		...
		for f in range(len(testfiles)):
			...
			with torch.no_grad():
				img_c = cv2.imread(test_dir + '/' + testfiles[f])
				h,w,c = img_c.shape
				img_c = cv2.resize(img_c,(512,512)) / 255.0         
