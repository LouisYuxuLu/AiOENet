...
preNet = my_vgg16(numClass=3)
preNet.load_state_dict(torch.load("./ckp/model.pth", map_location=device))
CatAndDog = myDataSet("./test/", data_transform)
cadSet = torch.utils.data.DataLoader(CatAndDog, batch_size=batch_size, shuffle=True, num_workers=0)
allYes = 0
preNet.cuda()
